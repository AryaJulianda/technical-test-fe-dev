...
const MyCard = ({data}) => {
  ...
  return (
    <div className="bg-white w-max flex-1 flex flex-col gap-5 p-6 rounded-xl shadow-lg // max-xl:p-5 // max-lg:p-4 max-lg:gap-4">
    ...
    <div className="flex flex-row justify-between">
      <div>
        ...
        <p className="text-base font-medium text-[#aaa] // max-lg:text-sm">{data && data[selectedCard]?.history_month}</p>
      </div>
      <button className="text-base font-medium text-primary-blue cursor-pointer // max-lg:text-sm ">See All</button>
    </div>

    <div className="flex flex-col gap-3">
      {!data[selectedCard]?.history.length == 0 ?
        data[selectedCard]?.history?.map((value,index) => {
          return (
          ...
          <div className="flex flex-row justify-between p-4 rounded-xl bg-[#f0f0f0] items-center " key={index}>
            ...
            <div>
              <h1 className={value.category == 'income' ? "text-xl text-[#7DD97B] font-semibold // max-lg:text-lg" : "text-xl text-[#F3735E] font-semibold // max-lg:text-lg"}>
                ${value.amount}
              </h1>
            </div>
          </div>
